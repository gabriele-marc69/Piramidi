...
## Core Idea
Before presenting newly discovered structures, the authors validate the MM tomography method by re-imaging the *already known* interior features (King's Chamber, Zed, Queen's Chamber, Grand Gallery, Grotto, unfinished room) and checking that the tomograms line up with the accepted architectural diagram.
...
## Key Concepts
- **Tomographic line orientation**: the internal tomography in this paper uses **6 acquisitions** — Table 2 pictures 7 to 12, stated explicitly as the internal-results input. They span only **3 distinct orbit/beam geometries** (left-descending beam 46; right-descending beam 48; right-ascending beam 48); the variety of sides and angles comes mostly from choosing different tomographic lines on those images, not from more passes. No single line can reveal the whole interior.
...
## Key Takeaways
1. Known structures (King's Chamber, Zed, Queen's Chamber, Grand Gallery, Grotto, unfinished room) were all recovered by the tomography, which the authors present as their core validation step.
...
3. Six acquisitions (Table 2, pictures 7–12) across three orbit/beam geometries, plus multiple tomographic-line orientations per image, build the composite internal 3D picture — no single pass or single line suffices.
